    ...
    var body: some View {
        VStack {
            TrailField(iconName: "figure.hiking", label: "Distance") {
                TextField("kilometres", value: $trailInfo.distance, format: .number)
                    .keyboardType(.numberPad)
                    .multilineTextAlignment(.trailing)
            }
            
            TrailField(iconName: "mountain.2.fill", label: "Elevation Change") {
                TextField("metres", value: $trailInfo.elevation, format: .number)
                    .keyboardType(.numberPad)
                    .multilineTextAlignment(.trailing)
            }
            ...
            TrailField(iconName: "exclamationmark.triangle.fill", label: "Danger fom wildlife") {
